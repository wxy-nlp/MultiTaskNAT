...
def save_checkpoint(cfg: CheckpointConfig, trainer, epoch_itr, val_loss):
    ...
    if val_loss is not None and cfg.keep_best_checkpoints > 0:
        checkpoint_conds[
            "checkpoint.top{}_{}_{:.2f}.pt".format(cfg.keep_best_checkpoints,
    ...
    if cfg.keep_best_checkpoints > 0:
        # only keep the top N checkpoints according to validation metric
        checkpoints = checkpoint_paths(
            cfg.save_dir,
            pattern=r"checkpoint\.top{}_{}_(\d+\.?\d*)\.pt".format(
                cfg.keep_best_checkpoints, cfg.best_checkpoint_metric
            ),
        )
        if not cfg.maximize_best_checkpoint_metric:
            checkpoints = checkpoints[::-1]
...
def load_model_ensemble_and_task(
    ...
    for filename in filenames:
        ...
        for shard_idx in range(num_shards):
            ...
            # if task is None:
            task = tasks.setup_task(cfg.task, model=cfg.model)
