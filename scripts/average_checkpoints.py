...
def last_n_checkpoints(paths, n, metric, update_based, best_based, top_based, upper_bound=None):
    assert len(paths) == 1
    path = paths[0]
    if update_based:
        pt_regexp = re.compile(r'checkpoint_\d+_(\d+)\.pt')
    elif best_based:
        pt_regexp = re.compile(r'checkpoint.best_{}_(\d+\.\d+)\.pt'.format(metric))
    elif top_based:
        pt_regexp = re.compile(r'checkpoint.top{}_{}_(\d+\.\d+)\.pt'.format(n, metric))
    else:
...
def main():
    ...
    num_group.add_argument('--num-best-checkpoints', type=int,
                           help='if set, will try to find checkpoints with names checkpoint_ee_xx.pt in the path specified by input, '
                           'and average last this many of them.')
    num_group.add_argument('--num-top-checkpoints', type=int)
    num_group.add_argument('--checkpoint-metric', type=str, default='bleu',
                           help="if set, will assign best checkpoint metric.")
    ...
    elif args.num_top_checkpoints is not None:
        is_top_based = True
        num = args.num_top_checkpoints
        metric = args.checkpoint_metric
